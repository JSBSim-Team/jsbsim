...
A Windows installer `JSBSim-1.1.9-setup.exe` is available in the [release section](https://github.com/JSBSim-Team/jsbsim/releases/tag/v1.1.9). It installs the 2 executables along with aircraft data and some example scripts:
...
Debian packages for Ubuntu Linux "Bionic" 18.04 LTS and "Focal" 20.04 LTS for 64 bits platforms are also available in the [JSBSim project release section](https://github.com/JSBSim-Team/jsbsim/releases/tag/v1.1.9). There are 3 packages for each platform:

* `JSBSim_1.1.9-623.amd64.deb` which installs the executables `JSBSim` and `aeromatic`
* `JSBSim-devel_1.1.9-623.amd64.deb` which installs the development resources (headers and libraries)
* `python3-JSBSim_1.1.9-623.amd64.deb` which installs the Python 3.6 module of JSBSim
