    ...
    <xsl:template match="/fdm_config">
        <html>
            ...
            <body style="font-family:Arial;font-size:90%">
                ...
                <xsl:if test="ground_reactions">[<a href="#groundreactions">Ground Reactions</a>] </xsl:if>
...
<xsl:if test="metrics">
                    ...
                    <tr bgcolor="DDEEFF">
                        <table width="100%" style="font-family:Arial; font-size:90%" bgcolor="EEEEEE" cellpadding="0" cellspacing="0">
                            ...
                            <!-- wingspan -->
                            <tr><td width="300"><b>Wing span <xsl:if test="metrics/wingspan/@unit">(<xsl:value-of select="metrics/wingspan/@unit"/>)</xsl:if></b></td>
                                <td align="left"><xsl:value-of select="metrics/wingspan"/></td></tr>
                            <!-- Wing area -->
                            <tr><td width="300"><b>Wing area <xsl:if test="metrics/wingarea/@unit">(<xsl:value-of select="metrics/wingarea/@unit"/>)</xsl:if></b></td>
                                <td><xsl:value-of select="metrics/wingarea"/></td></tr>
                            <!-- Chord -->
                            <tr><td width="300"><b>Chord <xsl:if test="metrics/chord/@unit">(<xsl:value-of select="metrics/chord/@unit"/>)</xsl:if></b></td>
                                <td><xsl:value-of select="metrics/chord"/></td></tr>
                            <!-- horizontal tail area -->
                            <xsl:if test="metrics/htailarea">
                                <tr><td width="300"><b>Horizontal tail area <xsl:if test="metrics/htailarea/@unit">(<xsl:value-of select="metrics/htailarea/@unit"/>)</xsl:if></b></td>
                                <td><xsl:value-of select="metrics/htailarea"/></td></tr>
                              </xsl:if>
                            <!-- Horizontal tail arm -->
                            <xsl:if test="metrics/htailarm">
                                <tr><td width="300"><b>Horizontal tail arm <xsl:if test="metrics/htailarm/@unit">(<xsl:value-of select="metrics/htailarm/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="metrics/htailarm"/></td></tr>
                            </xsl:if>
                            <!-- Horizontal tail area -->
                            <xsl:if test="metrics/htailarea">
                                <tr><td width="300"><b>Vertical tail area <xsl:if test="metrics/vtailarea/@unit">(<xsl:value-of select="metrics/vtailarea/@unit"/>)</xsl:if></b></td>
                                <td><xsl:value-of select="metrics/vtailarea"/></td></tr>
                            </xsl:if>
                            <!-- Vertical tail arm -->
                            <xsl:if test="metrics/vtailarm">
                                <tr><td width="300"><b>Vertical tail arm <xsl:if test="metrics/vtailarm/@unit">(<xsl:value-of select="metrics/vtailarm/@unit"/>)</xsl:if></b></td>
                    ...
                    <tr bgcolor="DDEEFF">
                        <table width="100%" style="font-family:Arial; font-size:90%" bgcolor="EEEEEE" cellpadding="0" cellspacing="0">
                            ...
                            <xsl:if test="mass_balance/ixx">
                                <tr><td width="300"><b>Ixx <xsl:if test="mass_balance/ixx/@unit">(<xsl:value-of select="mass_balance/ixx/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="mass_balance/ixx"/></td></tr>
                            </xsl:if>
                            <xsl:if test="mass_balance/iyy">
                                <tr><td width="300"><b>Iyy <xsl:if test="mass_balance/iyy/@unit">(<xsl:value-of select="mass_balance/iyy/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="mass_balance/iyy"/></td></tr>
                            </xsl:if>
                            <xsl:if test="mass_balance/izz">
                                <tr><td width="300"><b>Izz <xsl:if test="mass_balance/izz/@unit">(<xsl:value-of select="mass_balance/izz/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="mass_balance/izz"/></td></tr>
                            </xsl:if>
                            <xsl:if test="mass_balance/ixy">
                                <tr><td width="300"><b>Ixy <xsl:if test="mass_balance/ixy/@unit">(<xsl:value-of select="mass_balance/ixy/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="mass_balance/ixy"/></td></tr>
                            </xsl:if>
                            <xsl:if test="mass_balance/ixz">
                                <tr><td width="300"><b>Ixz <xsl:if test="mass_balance/ixz/@unit">(<xsl:value-of select="mass_balance/ixz/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="mass_balance/ixz"/></td></tr>
                            </xsl:if>
                            <xsl:if test="mass_balance/iyz">
                                <tr><td width="300"><b>Iyz <xsl:if test="mass_balance/iyz/@unit">(<xsl:value-of select="mass_balance/iyz/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="mass_balance/iyz"/></td></tr>
                            </xsl:if>
                            <xsl:if test="mass_balance/emptywt">
                                <tr><td width="300"><b>Empty Weight <xsl:if test="mass_balance/emptywt/@unit">(<xsl:value-of select="mass_balance/emptywt/@unit"/>)</xsl:if></b></td>
                                    <td><xsl:value-of select="mass_balance/emptywt"/></td></tr>
                            </xsl:if>
...
<xsl:if test="ground_reactions">
				...
				<xsl:for-each select="ground_reactions/contact">
				    <xsl:if test="@type='BOGEY'">
					  ...
					  <u>Rolling friction coefficient</u>: <xsl:value-of select="rolling_friction"/><br/>
					  <u>Spring constant<xsl:if test="spring_coeff/@unit"> (<xsl:value-of select="spring_coeff/@unit"/>)</xsl:if></u>: <xsl:value-of select="spring_coeff"/><br/>
					  <u>Damping constant<xsl:if test="damping_coeff/@unit"> (<xsl:value-of select="damping_coeff/@unit"/>)</xsl:if></u>: <xsl:value-of select="damping_coeff"/><br/>
					  <u>Maximum steering angle<xsl:if test="max_steer/@unit"> (<xsl:value-of select="max_steer/@unit"/>)</xsl:if></u>: <xsl:value-of select="max_steer"/><br/>
					</td></tr>
				    </xsl:if>
				</xsl:for-each>
				<xsl:for-each select="ground_reactions/contact">
				    <xsl:if test="@type=STRUCTURE">
				        <tr><td>Contact point: <xsl:value-of select="@name"/></td><td>Stuff</td></tr>
				    </xsl:if>
				</xsl:for-each>
                            </table>
                    </tr>
                                        ...
                                        <font face="Arial" size="2">
                                            ...
                                            <xsl:choose>
                                                <xsl:when test="name() = 'lag_filter'">
                                                    ...
                                                    <table>
                                                        <tr><td nowrap="1" valign="center" align="left"><font face="Arial" size="2">
							Filter:</font></td><td align="center" nowrap="1"><font face="Arial" size="2">
                                                        <xsl:value-of select="c1"/><hr width="100%"/>
						    ...
						    Trigger property: <xsl:value-of select="trigger"/>
                                                ...
                                                <xsl:when test="name() =  'second_order_filter'" >
                                                    ...
                                                    <table>
                                                        ...
                                                        <xsl:value-of select="c2"/>s + <xsl:value-of select="c3"/><hr width="100%"/>
...
<xsl:if test="aerodynamics">
                    ...
                    <tr bgcolor="CCCCCC">
                            <table width="100%" style="font-family:Arial; font-size:90%" bgcolor="EEEEEE" cellpadding="0" cellspacing="0">
                                ...
                                <xsl:if test="count(aerodynamics/function) >0"><tr><td colspan="3"><b>Global Functions</b></td></tr>
...
<!-- Process products/tables, etc. -->
    ...
    <xsl:template name="parse_property">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="parse_property">
                    <xsl:with-param name="property_string" select="substring-after($property_string,'/')"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
