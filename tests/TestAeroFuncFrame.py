...
class TestAeroFuncFrame(JSBSimTestCase):
    def setUp(self):
        ...
        self.aero2wind = np.array(np.identity(3));
    ...
    def checkForcesAndMoments(self, getForces, getMoment, aeroFunc):
        ...
        rp = np.array([self.fdm['metrics/aero-rp-x-in'],
        ...
        while self.fdm.run():
            ...
            cg = np.array([self.fdm['inertia/cg-x-in'],
    ...
    def checkBodyFrame(self, frame):
        ...
        def getForces(result):
            Tb2w = self.auxilliary.get_Tb2w()
            Fb = np.array([result['X'], result['Y'], result['Z']]).T
            Fw = Tb2w * Fb
            Fa = self.aero2wind * Fw
            return Fa, Fb

        def getMoment(result):
            return np.array([result['ROLL'], result['PITCH'], result['YAW']])
    ...
    def testAxialFrame(self):
        ...
        def getForces(result):
            Tb2w = self.auxilliary.get_Tb2w()
            Fnative = np.array([result['AXIAL'], result['SIDE'], result['NORMAL']]).T
    ...
    def testWindFrame(self):
        ...
        def getForces(result):
            ...
            Fa = np.array([result['X'], result['Y'], result['Z']]).T
        ...
        def getMoment(result):
            Tw2b = self.auxilliary.get_Tw2b()
            Mw = np.array([result['ROLL'], result['PITCH'], result['YAW']]).T
            Mb = Tw2b*Mw
            return Mb.T
    ...
    def testAeroFrame(self):
        ...
        def getForces(result):
            Tw2b = self.auxilliary.get_Tw2b()
            Fa = np.array([result['DRAG'], result['SIDE'], result['LIFT']]).T
            Fw = self.aero2wind * Fa
            Fb = Tw2b * Fw
            return Fa, Fb
    ...
    def testStabilityFrame(self):
        ...
        def getForces(result):
            Tb2w = self.auxilliary.get_Tb2w()
            Ts2b = self.getTs2b()
            Fs = np.array([result['X'], result['Y'], result['Z']]).T
        ...
        def getMoment(result):
            Ts2b = self.getTs2b()
            Ms = np.array([result['ROLL'], result['PITCH'], result['YAW']]).T
            Mb = Ts2b*Ms
            return Mb.T
